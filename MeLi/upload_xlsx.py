            ...
            if str(type(forma_entrega)) == "<class 'NoneType'>" or str(type(forma_entrega)) == "<type 'NoneType'>":
                ...
                if "Flex" in forma_entrega:
                    ...
                    if nro_seguimiento in nros_envios:
                        ...
                        sql =("insert into ViajesFlexs (Fecha, Numero_envío,nro_venta,comprador,Direccion,Referencia,Localidad,CP,Vendedor,estado_envio, Direccion_completa, Latitud, Longitud,tipo_envio) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)")
                        values = (fecha_hoy,nro_seguimiento,nro_venta,comprador,direccion,referencia,ciudad,cp,vendedor,"pendiente de retiro",direccion_completa,-34.5,-58.5,"Flex")
                        cursor.execute(sql,values)
                        midb.commit()
                        lista_viajes.append(paquete)
        midb.close()
        return render_template("data.html",titulo="Carga", data=lista_viajes, analizados=cant_viajes, agregados=flex_agregado, flex=flex, repetido=repetido, auth = session.get("user_auth"))
    else:
        return render_template("carga_archivo.html",titulo="Carga", auth = session.get("user_auth"), clientes=obtenerClientes(), url_post="upload")
