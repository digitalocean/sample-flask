...
def sueldoChofer():
    ...
    if request.method == "GET":
        ...
        viajes = []
        sql = f"SELECT {columnasConsulta} FROM mmslogis_MMSPack.sueldo where Chofer = '{chofer}' and Fecha between '{desde}' and '{hasta}' order by Fecha union SELECT {columnasConsulta} FROM mmslogis_MMSPack.sabado where Chofer = '{chofer}' and motivo_noenvio in ('Entregado sin novedades','Nadie en Domicilio (Reprogramado)','Rechazado por el comprador') order by Fecha;"
        cursor.execute(sql)
        viajeSabado = 0
        suma = 0
        cantidad = 0
