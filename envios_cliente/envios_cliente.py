...
def envios_clientes():
    ...
    cursor = midb.cursor()
    cabezeras = ["Fecha","Numero de envío","Numero de venta","Comprador","Direccion","Estado","Motivo","Observación"]
    cursor.execute("select Fecha, Numero_envío, nro_venta, comprador,concat(Direccion,', ',Localidad), estado_envio,Motivo,Observacion from ViajesFlexs where Vendedor = %s order by Fecha Desc", (session.get("user_id"),))
    for viaje in cursor:
        viajes.append(viaje)
